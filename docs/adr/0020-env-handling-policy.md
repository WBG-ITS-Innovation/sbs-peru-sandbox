...
3. **Developer local `.env`.** Each developer copies `.env.example` to `.env` and fills in real values. The four `AZURE_OPENAI_*` variables are the only mandatory ones at Part 1 (see [ADR 0015](0015-cross-review-llm-backend-azure.md) for the Azure OpenAI backend decision — written 2026-08-13, having been cited here while still unwritten); more variables join as the application stack lands.

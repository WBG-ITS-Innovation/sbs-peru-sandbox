...
In demo mode, the persona switcher changes the active persona without re-running the landing rule. Switching to the Conduct Unit Head from the Conduct Supervisor's cockpit does not redirect to `/app/approvals`; the operator stays on the current screen with the Conduct Unit Head's identity. This matches the demo flow: the narrator wants to walk through cockpit → findings → approvals as one continuous experience, not three sign-in cycles or three landings.
...
- The audit row on login carries the landing decision. the Superintendent can answer "where did each user land on May 25?" by reading the audit table; no separate telemetry needed.

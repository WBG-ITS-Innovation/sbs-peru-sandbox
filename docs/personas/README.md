...
Personas are **role aliases** — job descriptions, not individuals.
Two — **Head** and **ITOps** — replaced earlier placeholders that
collided with real SBS staff names (P-RESHAPE-8.6).

| Persona | Display name | Role string | Stub id | What they do |
|---|---|---|---|---|
| Analyst | Analyst Demo | `sbs:conduct:analyst` | `analyst` | Inspect individual complaints; model internals; corpus search |
| Supervisor | Supervisor Lazo | `sbs:conduct:supervisor` | `supervisor` | Pattern landscape for assigned institutions; approve FIBriefs |
| Unit Head | Head Caballero | `sbs:conduct:head` | `head` | All patterns; final approval + override authority; audit |
| Superintendent | Superintendent Velarde | `sbs:superintendent` | `superintendent` | Executive aggregates only — NO per-complaint data, NO raw narrative |
| SBS IT | ITOps Salazar | `sbs:sbs_it` | `itops` | Platform operations telemetry only — NO business data of any kind |
...
| Persona | Actions |
|---|---|
| Analyst | Propose pattern · Flag complaint for review · Request enrichment |
| Supervisor | Approve FI brief · Delegate pattern to analyst · Defer pattern |
| Unit Head | Override supervisor decision · Approve sector broadcast (primary) · Generate weekly digest |
| Superintendent | Co-approve sector broadcast (secondary) · Request deeper look · Sign weekly digest |
| SBS IT (`itops`) | Annotate incident · Retry failed webhook · Requeue agent run · Circuit-break FI ingestion |
...
`<persona>-demo-2026`, e.g. `head-demo-2026`.
...
where `<persona_id>` is one of `analyst`, `supervisor`, `head`, `superintendent`,
`itops`. The token resolves to that persona's role and exact scope set —
...
```bash
# Head holds agents:read → 200
curl -H "Authorization: Bearer stub:head" \
  http://localhost:8000/v1/internal/cockpit/agents/divalevale/activity

# Superintendent holds no institution complaints scope → 403 (scope enforced)
curl -H "Authorization: Bearer stub:superintendent" \

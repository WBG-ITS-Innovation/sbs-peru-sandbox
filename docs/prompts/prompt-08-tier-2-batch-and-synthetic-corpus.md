...
3. **No squash-merge for multi-workstream PRs.** Per the post-Prompt-7 conversation with Antoine, the repo default for multi-workstream PRs is now rebase-merge. This spec assumes that's in place; if it isn't, the maintainer toggles it before opening the PR.

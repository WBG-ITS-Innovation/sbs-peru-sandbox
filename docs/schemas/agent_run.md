...
| `model_provider` | `text` | yes | Provider that ACTUALLY served this run's model calls: one of `on_prem`, `replay`, `mock`, `cloud`. Recorded because `OnPremProvider` falls back to `MockProvider` whenever no vLLM endpoint answers — without this column a mock-served run is indistinguishable from a model-served one, which the reconstructability contract in ADR 0001 depends on. The sentinel `none` means the run made no model call by design — the live-ingestion orchestrator's row, whose work is the deterministic anonymizer and data-quality pass. Null means only that the row predates migration `20260810_0001`, which added the column without backfilling. Before v0.2.0 null carried both meanings, so `model_provider IS NOT NULL` — the filter the operator docs prescribe — silently dropped current successful analysis; see `docs/audit/2026-08-17-v02-check.md` F3. No `status='success'` row is written with a null provider. |

...
# The Host header curl actually sends (with port) is what the server
# canonicalises against. The HMAC canonical request must sign the
# same string. See ADR 0027 amendment §canonical-request line 3.
...
# Canonical request lines: method / path / lowercased-host-header / timestamp / body-hash / institution_id

...
@pytest.mark.asyncio
async def test_stub_jorge_reaches_internal_cockpit(dev_stub_app):
    ...
    # Jorge holds agents:read → 200 (not 401, not 404).
    assert r.status_code == 200


@pytest.mark.asyncio
async def test_stub_rosa_reaches_ops_but_not_business(dev_stub_app):
    transport = ASGITransport(app=dev_stub_app)
    async with AsyncClient(transport=transport, base_url="http://test") as c:
        ops = await c.get(
            "/v1/internal/ops/ingestion_lag",
            headers={"Authorization": "Bearer stub:rosa"},
        )
        exec_ = await c.get(
            "/v1/internal/exec/cohort_health",
            headers={"Authorization": "Bearer stub:rosa"},
        )
    assert ops.status_code == 200  # rosa has ops:read
    assert exec_.status_code == 403  # rosa has no exec:read
...
@pytest.mark.asyncio
async def test_stub_unknown_persona_401_over_http(dev_stub_app):
    transport = ASGITransport(app=dev_stub_app)
    async with AsyncClient(transport=transport, base_url="http://test") as c:
        r = await c.get(
            "/v1/internal/cockpit/agents/divalevale/activity",
            headers={"Authorization": "Bearer stub:ghost"},
        )
    assert r.status_code == 401


@pytest.mark.asyncio
async def test_real_shared_secret_still_works_alongside_stub(dev_stub_app):
    transport = ASGITransport(app=dev_stub_app)
    async with AsyncClient(transport=transport, base_url="http://test") as c:
        r = await c.get(
            "/v1/internal/cockpit/agents/divalevale/activity",
    ...
    # Real shared-secret + X-SBS-Role path coexists with the stub.
    assert r.status_code == 200
